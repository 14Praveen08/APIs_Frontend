...
export class AllFacultyComponent implements OnInit {
  ...
  ask;
  selectedInst;
  constructor(private orgService: ApiService,private _facultyService:FacultyService,private _activatedroute:ActivatedRoute) { 
  ...
  load(){
    this._facultyService.getAllFactulty().subscribe((data:FacultyObj[])=>{this.faculty=data,_.isEmpty(this.faculty)?this.flag=false:this.flag=true});
  }
  reload(){
